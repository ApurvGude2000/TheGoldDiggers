    ...
    function showTooltip(event, violation, nodeInfo) {
      ...
      const explanationHtml = `
        <div style="margin-bottom: 12px;">
          ...
          <div style="display: inline-block; background: ${SEVERITY_COLORS[violation.impact].border}; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; text-transform: uppercase; margin-bottom: 8px;">
            ${violation.impact}
          </div>
        </div>
  
        <div style="margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb;">
          <div style="font-weight: 600; font-size: 13px; margin-bottom: 6px;">What's wrong:</div>
          <div style="color: #666; font-size: 13px;">${violation.description}</div>
        </div>
  
        ${violation.mlAnalysis?.explanation ? `
          <div style="margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb;">
            <div style="font-weight: 600; font-size: 13px; margin-bottom: 6px;">Who this affects:</div>
            <div style="color: #666; font-size: 13px;">
              ${violation.mlAnalysis.explanation.who?.map(w => `• ${w}`).join('<br>') || 'Users with disabilities'}
            </div>
          </div>
  
          <div style="margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb;">
            <div style="font-weight: 600; font-size: 13px; margin-bottom: 6px;">Why it matters:</div>
            <div style="color: #666; font-size: 13px;">${violation.mlAnalysis.explanation.why || 'This violates WCAG accessibility guidelines'}</div>
          </div>
  
          <div style="margin-bottom: 8px;">
            <div style="font-weight: 600; font-size: 13px; margin-bottom: 6px; color: #059669;">How to fix:</div>
            <div style="color: #666; font-size: 13px;">
              ${violation.mlAnalysis.explanation.how?.map(h => `• ${h}`).join('<br>') || 'Review WCAG documentation'}
            </div>
          </div>
        ` : ''}
  
        ${violation.mlAnalysis?.impact ? `
          <div style="background: #f3f4f6; padding: 8px; border-radius: 4px; margin-top: 12px; font-size: 12px;">
            <strong>Estimated Impact:</strong> ${violation.mlAnalysis.impact.percentage}% of users
          </div>
        ` : ''}
  
        <div style="margin-top: 12px; font-size: 11px; color: #999;">
          <a href="${violation.helpUrl}" target="_blank" style="color: #3b82f6; text-decoration: none;">Learn more →</a>
        </div>
      `;
